...
sequenceDiagram
    ...
    User->>LLM: "查一下苹果15的库存"
    LLM->>Aspect: 调用 queryStock("苹果15")
    
    Note over Aspect: 🛑 拦截请求
    
    Aspect->>Search: fuzzySearch("苹果15")
    Search->>Search: 在Map中匹配<br/>"iPhone 15" -> "P-001"
    Search-->>Aspect: [{id:"P-001", name:"iPhone 15"}]
    
    Note over Aspect: ✅ 唯一匹配,自动矫正
    
    Aspect->>Service: queryStock("P-001")
    Service->>DB: SELECT stock WHERE id='P-001'
    DB-->>Service: 100
    Service-->>Aspect: "库存: 100"
    Aspect-->>LLM: "库存: 100"
    LLM-->>User: "iPhone 15 的库存是 100"
```
...
sequenceDiagram
    ...
    Note over Tool: 🛑 检查 confirmed=false
    
    Tool->>Tool: 生成确认单<br/>(不执行业务)
    Tool-->>LLM: "⚠️ 请确认操作..."
    LLM-->>User: "您确认要发货吗？"
    ...
    Note over Tool: ✅ 检查 confirmed=true
